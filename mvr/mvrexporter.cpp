...
static std::pair<int,int> ParseAddress(const std::string& addr)
{
    ...
    if (dot == std::string::npos)
        return {0,0};
    int u = 0, c = 0;
    try { u = std::stoi(addr.substr(0,dot)); } catch(...) {}
    try { c = std::stoi(addr.substr(dot+1)); } catch(...) {}
    return {u,c};
}
...
bool MvrExporter::ExportToFile(const std::string& filePath)
{
    ...
    for (const std::string& rel : resourceFiles) {
        ...
        fs::path entryName = fs::path(rel).is_absolute() ? fs::path(rel).filename() : fs::path(rel);
