...
func _apply_emitter_light_state(light: SpotLight3D, photometric: Dictionary, normalized_dimmer: float, controls: Dictionary = {}) -> void:
	...
	if _fixture_gobo_projector != null:
		...
		var previous_applied_gobo_rotation_deg: float = float(light.get_meta("peraviz_beam_last_gobo_rotation_deg", beam_params.get("gobo_rotation_deg", 0.0)))
		...
		light.set_meta("peraviz_beam_last_gobo_rotation_deg", applied_gobo_rotation_deg)
		beam_params["gobo_rotation_deg"] = applied_gobo_rotation_deg
		if absf(applied_gobo_rotation_deg - previous_applied_gobo_rotation_deg) > 0.001:
			gobo_projection_changed = true
