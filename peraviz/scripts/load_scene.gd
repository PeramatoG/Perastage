...
var _visual_settings := {
	...
	"light_volumetric_fog_energy": 0.55,
...
func _apply_visual_settings(settings: Dictionary) -> void:
	...
	if world_environment != null and world_environment.environment != null:
		...
		world_environment.environment.volumetric_fog_enabled = USE_GLOBAL_VOLUMETRIC_FOG
		world_environment.environment.volumetric_fog_density = float(_visual_settings.get("volumetric_fog_density", 0.0)) if USE_GLOBAL_VOLUMETRIC_FOG else 0.0

	if USE_GLOBAL_VOLUMETRIC_FOG:
...
func _resolve_light_volumetric_fog_energy() -> float:
	if not USE_GLOBAL_VOLUMETRIC_FOG:
		return 0.0
	return float(_visual_settings.get("light_volumetric_fog_energy", 1.0))
...
func _update_beam_for_light(light: SpotLight3D, beam_params: Dictionary) -> void:
	...
	if USE_GLOBAL_VOLUMETRIC_FOG and _active_beam_mode == BEAM_RENDER_MODE_VOLUMETRIC:
		var has_local_gobo_fallback: bool = USE_LOCAL_GOBO_BEAM_FALLBACK and bool(beam_params.get("has_gobo", false))
		if not has_local_gobo_fallback:
