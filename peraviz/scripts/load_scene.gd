...
var _visual_settings := {
	...
	"light_volumetric_fog_energy": 80.0,
	"use_local_fog_volume": true,
	"local_fog_density": 0.01,
	"local_fog_shape": 2,
...
const GOBO_SHADOW_COOKIE_SHADOW_BIAS: float = 0.01
const GOBO_SHADOW_COOKIE_SHADOW_NORMAL_BIAS: float = 0.1
const GOBO_SHADOW_COOKIE_SHADOW_BLUR: float = 0.01
...
func _ready() -> void:
	...
	camera.cull_mask = camera.cull_mask & ~(1 << 19)
...
func _apply_emitter_light_state(light: SpotLight3D, photometric: Dictionary, normalized_dimmer: float, controls: Dictionary = {}) -> void:
	...
	beam_params["local_fog_density"] = float(_visual_settings.get("local_fog_density", 0.01))
	beam_params["local_fog_shape"] = int(_visual_settings.get("local_fog_shape", 2))
	_update_beam_for_light(light, beam_params)
