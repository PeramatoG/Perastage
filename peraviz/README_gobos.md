...
- Peraviz now auto-applies gobo fog quality defaults when a native cookie path is active (project setting `peraviz_auto_gobo_fog_quality`, default `true`): positional shadow atlas at least `8192`, volumetric fog `volume_size` at least `384`, and fog `volume_depth` at most `48` to reduce visible froxel grid borders.
