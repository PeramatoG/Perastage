...
bool LoadTtfFontMetrics(const std::filesystem::path &path, TtfFontMetrics &metrics) {
  metrics = TtfFontMetrics{};
  std::string data;
  if (!ReadFileToString(path, data) || data.size() < 12)
    return false;

  uint32_t headOffset = 0, headLength = 0, hheaOffset = 0, hheaLength = 0;
  uint32_t maxpOffset = 0, maxpLength = 0, hmtxOffset = 0, hmtxLength = 0;
  uint32_t cmapOffset = 0, cmapLength = 0, os2Offset = 0, os2Length = 0;
  if (!FindTable(data, MakeTag('h','e','a','d'), headOffset, headLength) ||
      !FindTable(data, MakeTag('h','h','e','a'), hheaOffset, hheaLength) ||
      !FindTable(data, MakeTag('m','a','x','p'), maxpOffset, maxpLength) ||
      !FindTable(data, MakeTag('h','m','t','x'), hmtxOffset, hmtxLength) ||
      !FindTable(data, MakeTag('c','m','a','p'), cmapOffset, cmapLength))
    return false;
  FindTable(data, MakeTag('O','S','/','2'), os2Offset, os2Length);

  if (headOffset + 54 > data.size() || hheaOffset + 36 > data.size() || maxpOffset + 6 > data.size())
    return false;
  ...
  if (numGlyphs == 0 || numHMetrics == 0 || hmtxOffset + static_cast<uint32_t>(numHMetrics) * 4 > data.size())
    return false;

  std::vector<int> advanceWidths(numGlyphs, 0);
  int lastAdvance = 0;
  for (uint16_t i = 0; i < numHMetrics; ++i) {
    int advance = ReadU16(data, hmtxOffset + static_cast<size_t>(i) * 4);
  ...
  const std::string cmapData = data.substr(cmapOffset, std::min<size_t>(cmapLength, data.size() - cmapOffset));
  if (cmapData.size() < 4)
    return false;
  ...
  for (uint16_t i = 0; i < cmapTables; ++i) {
    size_t record = 4 + i * 8;
    if (record + 8 > cmapData.size())
      return false;
    uint16_t platformId = ReadU16(cmapData, record);
    uint16_t encodingId = ReadU16(cmapData, record + 2);
    uint32_t subOffset = ReadU32(cmapData, record + 4);
    if (subOffset + 2 > cmapData.size())
      continue;
    uint16_t format = ReadU16(cmapData, subOffset);
  ...
  auto glyphForCodepoint = [&](uint16_t code) -> uint16_t {
    for (uint16_t i = 0; i < segCount; ++i) {
      ...
      size_t glyphOffset = idRangeOffsetOffset + 2 * i + idRangeOffset + 2 * (code - startCount);
  ...
  for (size_t i = 0; i < metrics.advanceWidths.size(); ++i) {
    ...
    int advance = glyphIndex < advanceWidths.size() ? advanceWidths[glyphIndex] : missingWidth;
    metrics.advanceWidths[i] = advance;
    metrics.widths1000[i] = metrics.unitsPerEm > 0
...
const PdfFontDefinition *PdfFontCatalog::Resolve(const std::string &family) const {
  ...
  if (bold && lower.find("bold") != std::string::npos)
    return bold;
  if (lower.find("sans") != std::string::npos || lower.find("arial") != std::string::npos ||
...
std::string EncodeWinAnsi(const std::string &utf8) {
  ...
  while (i < utf8.size()) {
    ...
    if (lead < 0x80) { codepoint = lead; length = 1; }
      ...
      codepoint = ((lead & 0x1F) << 6) | (static_cast<unsigned char>(utf8[i + 1]) & 0x3F); length = 2;
    } else if ((lead >> 4) == 0xE && i + 2 < utf8.size()) {
      codepoint = ((lead & 0x0F) << 12) | ((static_cast<unsigned char>(utf8[i + 1]) & 0x3F) << 6) |
                  (static_cast<unsigned char>(utf8[i + 2]) & 0x3F);
      length = 3;
    } else if ((lead >> 3) == 0x1E && i + 3 < utf8.size()) {
      codepoint = ((lead & 0x07) << 18) | ((static_cast<unsigned char>(utf8[i + 1]) & 0x3F) << 12) |
...
double MeasureTextWidth(const std::string &text, double fontSize, const PdfFontDefinition *font) {
