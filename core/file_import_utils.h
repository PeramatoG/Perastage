...
inline void ProcessBlock(Sha256State &state, const uint8_t *data) {
  uint32_t w[64]{};
...
inline void UpdateSha256(Sha256State &state, const uint8_t *data, size_t len) {
...
inline std::string FinalizeSha256(Sha256State &state) {
...
ComputeFileSha256(const std::filesystem::path &path) {
  ...
  while (in.good()) {
    ...
    if (bytes > 0)
      detail::UpdateSha256(state, buf.data(), bytes);
  }
  return detail::FinalizeSha256(state);
}

inline std::string BuildStableRenamedFilename(const std::filesystem::path &path,
...
inline CopyResult CopyWithConflictPolicy(const std::filesystem::path &sourcePath,
