...
require('HasContainerColumns' in model_h + model and 'node->parent == nullptr' in model, 'Only root DMX Channel rows should expose container columns for channel functions.')
require('Freeze()' in modes and 'Thaw()' in modes, 'Browser resets must be frozen to avoid repaint flicker.')
