...
// Delegates the GDTF Share download command to its GUI workflow owner.
void MainWindow::OnDownloadGdtf(wxCommandEvent &WXUNUSED(event)) {
  ...
  const GdtfShareDownloadGuiCallbacks callbacks{
      ...
      [this](const std::string &path) { AddFixtureFromGdtfPath(path); }};
  RunGdtfShareDownloadWorkflow(this, configManager, callbacks);
}
