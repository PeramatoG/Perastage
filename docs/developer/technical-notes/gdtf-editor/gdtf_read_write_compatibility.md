...
The reusable GDTF editor model carries `FixtureType/@Description` as a UTF-8 document string. Empty and multiline descriptions are valid and are mutated through the same write-policy and derivative flow as physical properties. Description-only edits produce revision text that names FixtureType Description as the changed field.
