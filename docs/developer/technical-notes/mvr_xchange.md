...
- Canonical group-owned DNS-SD instances in the `<group>._mvrxchange._tcp.local.` form used by MVR 1.6 and libMVRgdtf.
...
1. Perastage publishes a PTR owned by `_mvrxchange._tcp.local.` whose target is the selected group instance, such as `Default._mvrxchange._tcp.local.`. SRV and TXT records are owned by that group instance; SRV targets a stable, station-specific `.local.` host name, and A resolves that host on the selected interface. TXT carries the user-facing `StationName` and persistent `StationUUID`.
...
PTR, SRV, TXT, A, and AAAA records retain their owner, target or address, TTL, interface identifier, responder address, last-seen time, and calculated expiry. The normative topology verified against MVR 1.6 and libMVRgdtf is `PTR _mvrxchange._tcp.local. -> <group>._mvrxchange._tcp.local.`, with group-owned SRV/TXT and a host-owned A/AAAA record. Multiple responders advertising the same group remain separate cache origins, preventing one station's TXT identity, SRV port, or host address from being combined with another station. Perastage also reads the previously emitted `<station>.<group>._mvrxchange._tcp.local.` instance-owned SRV/TXT layout when it is visibly scoped to the selected group; this is input compatibility, not normative output. Separate TXT strings in one DNS message merge case-insensitive keys, while a refreshed TXT RRset replaces the previous set. DNS identity is ASCII case-insensitive and ignores an optional trailing root dot while display spelling is preserved. TTL zero schedules the RFC 6762 one-second goodbye grace period; expiry removes the record from resolution, and station reconciliation clears stale incoming/outgoing handshake state. Absence is determined by TTL evidence rather than one unanswered query. IPv6 records are cached and displayed, although the current TCP transport still opens IPv4 sockets, so an AAAA-only peer is not yet reachable.

The MVR group-owned record model relies on responder/interface origin to distinguish stations that use the same group owner. StationUUID remains the protocol identity once TXT or JOIN data is available. Perastage's SRV host label is derived from the application name, machine name, and a stable StationUUID suffix rather than the user-facing StationName, avoiding collisions with another application on the same machine without changing the persisted StationUUID. Implementations that advertise distinct station-qualified instances remain readable. Multiple applications on one host that emit indistinguishable group-owned records from the same source address remain an mDNS representation ambiguity and require real-device validation; Perastage does not invent a proprietary wire format to resolve it.
