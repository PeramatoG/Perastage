...
The modal **2D View Editor** additionally routes `Z` fit, `←`, `→`, `↑`,
`↓`, and their `Alt` zoom variants through the embedded `Viewer2DPanel` even
when focus is in the editor side panels. Editable text controls keep priority
and receive their normal key input. This makes keyboard fit, pan, and zoom
behavior match the standalone 2D viewport while preserving text-editing safety.
