...
Each configure loads a generated CMake initial-cache file that safely bracket-quotes the exact action-provided executable and sets only the selected program and both language launchers. This avoids the PowerShell-to-Python-to-CMake tokenization that truncated Windows `sccache.exe`. Each job verifies those three entries, zeros job-local counters without deleting remote objects, and captures text and v0.15.0 JSON statistics under `out/ci-logs/sccache-<platform>-debug.*`. macOS exports and verifies `compile_commands.json`, as Linux and Windows already do. The Step Summary reports the configured backend and cache scope, namespace, compiler, normalized base directory, requests, executed requests, compilations, failures, hits, misses, writes, read/write errors, non-cacheable work, and hit rate. A successful build with zero requests or an unvalidated launcher fails. When configure or build already failed, the always-run statistics step reports that state without creating a second launcher failure. `SCCACHE_IGNORE_SERVER_IO_ERROR=1` lets transient backend I/O fall through to the real compiler, but installation, launcher, compiler, build, malformed-statistics, and test failures remain fatal. CTest always runs normally and its duration is uncached.

The PR run on 2026-07-25 proved native object caching on Linux with 1,814 requests, 1,200 hits, 614 misses, 602 writes, and 12 write errors, and on macOS with 15 requests, 15 misses, and 15 writes. Some Linux hits may be duplicate requests within the same build, but they still demonstrate real object reuse. The write errors were reported while compilation completed successfully under the intentional fail-open policy. The subsequent unchanged Linux CTest command ran the complete suite and exposed unrelated existing failures; compiler caching must not skip or weaken those tests.
