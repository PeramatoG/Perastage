...
The official web/XSD artifact could not be downloaded in this checkout: the web endpoint returned HTTP 401 and no XSD is vendored. Consequently no downloaded-XSD size or digest is claimed here; cross-platform PR verification must attach the authoritative XSD artifact if schema validation is required as D2 evidence.
...
The local container has no wxWidgets development package or preconfigured build directory, so wx-backed CTest reproduction and the complete Debug Tests workflow cannot run here. The GUI-independent `MvrIdentityRecovery` and `UuidUtilsRfc4122` sources were compiled directly and passed. D2 remains pending Linux and Windows CI results for both primary tests, the full failure-set comparison, and the reduced macOS result. Non-identity GDTF blockers in mixed MVR tests remain deferred.
