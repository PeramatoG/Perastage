...
To verify reuse, inspect each platform's sccache step summary in later pull requests and compare requests, hits, misses, writes, errors, compatibility identity, and build-step duration against the trusted `main` run. Performance improvement must be established from subsequent GitHub Actions measurements rather than inferred from this configuration change. CTest results are never cached, and they are skipped only by the trusted automatic warmer, so compare build-step duration separately.
