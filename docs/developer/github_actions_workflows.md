...
The 2026-07-25 PR run measured 1,814 Linux requests with 1,200 hits, 614 misses, 602 writes, and 12 write errors, plus 15 macOS requests, 15 misses, and 15 writes. Native object caching is therefore active; the next PR run must complete Windows compilation through the initial-cache launcher and complete the macOS summary. CTest commands and selections remain unchanged and uncached, including the complete Linux suite whose unrelated failures occur after successful compilation.
