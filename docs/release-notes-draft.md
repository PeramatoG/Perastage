...
- Finalized secure credential-storage release gates by unifying Windows vcpkg manifest installs, preventing duplicate CMake-time vcpkg installs, keeping Visual Studio presets usable before local tool provisioning, validating Windows Ninja x64 MSVC environments, adding focused credential tests to CI, and documenting Windows Credential Manager validation.
