...
- **Truss and support export/import** - MVR exports no longer embed local file paths, preserve layer colour metadata in a standards-compliant UserData block, and store fixture colours as gel/filter data only when intended.  Round-trip exports retain support objects, keep support-specific hoist metadata in root-level Perastage UserData instead of under Support nodes, and write truss and support children in the correct schema order.
