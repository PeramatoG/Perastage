...
- Finalized secure credential-storage release gates by unifying Windows vcpkg manifest installs, preventing duplicate CMake-time vcpkg installs, keeping Visual Studio presets usable before local tool provisioning, adding focused credential tests to CI, and documenting Windows Credential Manager validation.
