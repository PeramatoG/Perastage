...
- **Hoist load editing feedback** - the Load cell identifies manual values and automatic calculations based on missing weight data in red, explains the reason in its tooltip, and provides an explicit option to restore and recalculate the automatic value. The redundant Data Source column has been removed.
