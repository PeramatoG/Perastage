...
- Finalized secure credential-storage release gates by unifying Windows vcpkg manifest installs, allowing Visual Studio direct configure to install missing manifest dependencies while preventing duplicate CMake-time vcpkg installs in setup/CI, keeping Visual Studio presets usable before local tool provisioning, validating Windows Ninja x64 MSVC environments, adding focused credential tests to CI, and documenting Windows Credential Manager validation.
