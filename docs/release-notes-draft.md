...
- Added an Edit Truss dialog from double-clicking the Trusses table, including MVR instance fields, GDTF metadata, a reusable 3D preview, GDTF truss type fields, and automatic Perastage GDTF creation when type metadata is edited on a model-only truss.
