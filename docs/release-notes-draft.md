...
- Validated the native Linux clean-checkout developer workflow, completed its system dependency and locale setup, fixed external mDNS header discovery, corrected Windows wxWidgets secure-store validation for current vcpkg Debug and Release layouts, and kept local test artifacts out of the source tree; native Windows, Apple Silicon macOS, and WSL validation remains pending.
