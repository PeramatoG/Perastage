...
# Returns the explicit Bash value only when a caller supplied a non-empty usable value.
function(perastage_get_explicit_bash out_var)
    set(explicit_bash "")
    if(DEFINED BASH_EXECUTABLE AND NOT "${BASH_EXECUTABLE}" STREQUAL "" AND NOT "${BASH_EXECUTABLE}" MATCHES "-NOTFOUND$")
        set(explicit_bash "${BASH_EXECUTABLE}")
    endif()
    if(DEFINED CACHE{BASH_EXECUTABLE} AND NOT "$CACHE{BASH_EXECUTABLE}" STREQUAL "" AND NOT "$CACHE{BASH_EXECUTABLE}" MATCHES "-NOTFOUND$")
...
function(perastage_resolve_git_bash out_var)
    ...
    perastage_get_explicit_bash(explicit_bash)

    if(NOT is_windows)
        set(candidate "${explicit_bash}")
        if(NOT candidate)
            unset(_perastage_host_bash)
            find_program(_perastage_host_bash NAMES bash NO_CACHE)
            set(candidate "${_perastage_host_bash}")
        endif()
        ...
        if(RESOLVE_REQUIRED_FOR_TESTING)
            message(FATAL_ERROR "Bash is required for shell policy tests. Install bash or pass -DBASH_EXECUTABLE=<bash>; last probe: ${bash_message}")
        endif()
    ...
    unset(_perastage_git_executable)
    find_program(_perastage_git_executable NAMES git.exe git PATHS ENV PATH NO_DEFAULT_PATH NO_CACHE)
    perastage_git_for_windows_bash_candidates("${_perastage_git_executable}" candidates)
